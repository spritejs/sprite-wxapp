...
Page({
  ...
  onReady: function() {
    ...
    this.scene = scene
    const width = 750, height = 1344
  
    scene.preload(['../../assets/img/water.png', 
  ...
  moveTo: function(s, from, to) {
    ...
    return s.animate([
      ...
      duration: 100 * Math.max(Math.abs(from[0] - to[0])
        , Math.abs(from[1] - to[1])),
      fill: 'forwards',
    }).finished
  },
  shootDown: function(shooter, row, col) {
    const dropBoard = this.data.dropBoard
    let i
    for (i = row; i < 7; i++) {
  ...
  shootUp: function (shooter, row, col) {
    const dropBoard = this.data.dropBoard
    let i
    for (i = row; i >= 0; i--) {
      const drop = dropBoard[i][col]
  ...
  shootRight: function(shooter, row, col) {
    const dropBoard = this.data.dropBoard
    let i
    for (i = col; i < 6; i++) {
  ...
  shootLeft: function (shooter, row, col) {
    const dropBoard = this.data.dropBoard
    let i
    for (i = col; i >= 0; i--) {
      const drop = dropBoard[row][i]
  ...
  initDrops: function(reset = false) {
    if(reset) {
      this.updateDrops(10)
      this.data.score = 0
      this.addScore(0)
    ...
    return new Promise(resolve => {
      for (let i = drops.length - 1; i >= 0; i--) {
        ...
        if (idx !== i)
          [drops[idx], drops[i]] = [drops[i], drops[idx]]
        ...
        let waterDrop = dropBoard[row][col]
