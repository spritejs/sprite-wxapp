...
exports.options = exports.rerender = exports.render = exports.Component = exports.createRef = exports.cloneElement = exports.createElement = exports.use = exports.h = undefined;
...
var defer = typeof _promise2.default == 'function' ? _promise2.default.resolve().then.bind(_promise2.default.resolve()) : setTimeout;
...
var isBrowser = typeof window !== 'undefined' && window.document && window.document.documentElement;

function createNode(nodeName, attrs, isSvg) {
	if (!spritejs$1) throw new Error('No spritejs context.');
	var isValidNodeType = spritejs$1.isValidNodeType;
	var createSpriteNode = spritejs$1.createNode;

	var node = null;
	var isSpriteNode = isValidNodeType(nodeName) && (nodeName !== 'label' || attrs.text != null);
	...
	if (isSpriteNode && nodeName === 'scene') {
		...
		if (attrs.resources) {
			...
			(_node = node).preload.apply(_node, resources).then(function () {
				node.dispatchEvent('load', { resources: resources });
			});
		}
...
function removeNode(node) {
	...
	if (parentNode) parentNode.removeChild(node);else if (node.container && node.container.nodeType === 1) {
...
function appendNode(parent, node) {
	if (node.container && node.container.nodeType === 1) {
		var ret = parent.appendChild(node.container);
		node.parent = parent;
...
function idiff(dom, vnode, context, mountAll, componentRoot) {
	...
	if (!dom || !isNamedNode(dom, vnodeName)) {
		...
		if (dom) {
			while (dom.childNodes[0]) {
				appendNode(out, dom.childNodes[0]);
			}
			if (dom.parentNode) replaceNode(out, dom);
	...
	var fc = out.childNodes && out.childNodes[0],
	...
	if (props == null) {
		...
		if (out.nodeType === 1 || out.container && out.container.nodeType === 1) {
			var attributes = out.nodeType !== 1 && out.container ? out.container.attributes : out.attributes;
			for (var a = attributes, i = a.length; i--;) {
				props[a[i].name] = a[i].value;
			}
		} else {
			[].concat(out.attributes.__attributesSet).forEach(function (name) {
...
var _preact = __webpack_require__(159);
